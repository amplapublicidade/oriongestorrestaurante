...
const Products = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingProduct) {
      updateProduct({ id: editingProduct.id, productData: payload }, mutationOptions);
    } else {
      addProduct(payload, mutationOptions);
    }
  };
  ...
  const processExcelUpload = async () => {
    ...
    setIsUploading(true);
    setUploadProgress(0);
    console.log('🚀 Iniciando upload do Excel para o backend...');
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/products/upload-excel', {
        method: 'POST',
        headers: {
      ...
      toast.success(`Upload concluído! ${result.data.produtosCriados} produtos e ${result.data.fornecedoresCriados} fornecedores criados com sucesso.`);
